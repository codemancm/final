...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        \Log::info('=== REGISTER ATTEMPT ===', [
            'user_input' => $request->captcha,
            'session_code' => session('captcha_code'),
            'session_id' => session()->getId(),
            'session_has_captcha' => session()->has('captcha_code'),
        ...
        \Log::info('=== CAPTCHA COMPARISON ===', [
            'session_captcha' => $captchaCode,
            'user_input' => $userInput,
            'session_captcha_type' => gettype($captchaCode),
            'user_input_type' => gettype($userInput),
            'session_captcha_length' => strlen($captchaCode ?? ''),
            'user_input_length' => strlen($userInput ?? ''),
            'direct_comparison' => ($captchaCode === $userInput),
            'case_insensitive' => (strtoupper($captchaCode ?? '') === strtoupper($userInput ?? '')),
            'trimmed_comparison' => (trim(strtoupper($captchaCode ?? '')) === trim(strtoupper($userInput ?? '')))
        ]);

        // Check if session has captcha
        if (!$captchaCode) {
            \Log::error('NO CAPTCHA IN SESSION');
            return back()->with('error', 'No CAPTCHA found in session. Please refresh the page.')
                ->withInput($request->except('password', 'password_confirmation', 'reference_code'));
        }

        // Check if user provided captcha
        if (!$userInput) {
            \Log::error('USER PROVIDED NO CAPTCHA');
            return back()->with('error', 'Please enter the CAPTCHA code.')
                ->withInput($request->except('password', 'password_confirmation', 'reference_code'));
        }

        // Try different comparison methods
        $trimmedSessionCode = trim(strtoupper($captchaCode));
        $trimmedUserInput = trim(strtoupper($userInput));

        if ($trimmedSessionCode !== $trimmedUserInput) {
            \Log::error('CAPTCHA VALIDATION FAILED', [
                'expected_trimmed' => $trimmedSessionCode,
                'received_trimmed' => $trimmedUserInput,
                'expected_raw' => $captchaCode,
                'received_raw' => $userInput
            ]);
        
            return back()->with('error', "CAPTCHA mismatch. Expected: '{$trimmedSessionCode}', Got: '{$trimmedUserInput}'")
                ->withInput($request->except('password', 'password_confirmation', 'reference_code'));
        }
